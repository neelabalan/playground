...
const createBuildQueueTable = `-- name: CreateBuildQueueTable :exec
...
func (q *Queries) CreateBuildQueueTable(ctx context.Context) error {
	_, err := q.db.Exec(ctx, createBuildQueueTable)
	return err
}

const createBuildsTable = `-- name: CreateBuildsTable :exec
...
func (q *Queries) CreateBuildsTable(ctx context.Context) error {
	_, err := q.db.Exec(ctx, createBuildsTable)
	return err
}
