...
def display_task(task):
    ...
    console.print(
        Panel(
            ...
            title='[{}] {} [/]'.format(str(getattr(RichStyles.Status, task.get('status'))), task.get('status')),
            title_align='left',
            subtitle=subtitle,
...
def number_of_task_based_on_status():
    return {status: len(filter_tasks_by_status(status)) for status in states.possible_states()}
...
@app.command()
def view():
    task_name = pyskim.skim(get_all_task_name(), '--ansi --preview="tmt preview {}" --preview-window=up:50%')
    if task_name:
...
def find(searchstr: str):
    ...
    tasks = db.find(lambda x: searchstr in x.get('task') or searchstr in x.get('description'))
    for task in tasks:
        display_task(task)
