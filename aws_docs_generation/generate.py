...
class AwsServiceDocumenter:
    ...
    def traverse_and_document(
        self, shape: Shape, history: List[str] = None
    ) -> Dict[str, Any] | str:
        if history is None:
            history = []
            ...
            for key, _value in shape.members.items():
                if _value.type_name == "map" and hasattr(_value.value, "members"):
                    result[key] = self.traverse_and_document(
                        _value.value, history=history.copy()
                    )
                    ...
                    result[key] = {
                        ...
                        if hasattr(_value, "documentation")
                        else None,
    ...
    def process_method(self, method_name: str) -> Optional[Dict[str, str]]:
        ...
        try:
            if hasattr(operation_model.input_shape, "members"):
                method_map.update(
                    {
                        "method_request_schema": self.traverse_and_document(
            ...
            if hasattr(operation_model.output_shape, "members"):
                method_map.update(
                    {
                        "method_response_schema": self.traverse_and_document(
