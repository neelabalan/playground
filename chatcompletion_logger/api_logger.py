...
class ChatCompletionHttpLogger(AbstractHttpLogger):
    ...
    def log_response(self, response):
        ...
        prompt_token_cost= self.pricing_calculator.calculate_token_cost(model=model, token_count=response_json["usage"]["prompt_tokens"], token_type="input")
        completion_token_cost= self.pricing_calculator.calculate_token_cost(model=model, token_count=response_json["usage"]["completion_tokens"], token_type="output")
